    ...
    for offset in [16, 114]:
        ...
        for b in decoded:
            if 32 <= b <= 126:
                ...
                if len(curr) >= 4:
                    printable.append(curr.decode('ascii', errors='ignore'))
                curr = bytearray()
        if len(curr) >= 4:
            printable.append(curr.decode('ascii', errors='ignore'))
            
        print("  Printable strings:", printable)
        print("  First 48 bytes hex:", decoded[:48].hex())
