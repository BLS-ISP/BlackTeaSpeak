...
fn main() {
    let d0_seed_b64 = "QCUIVtjakIWe3BoNWHt9c6BX8lUyR4QOPirywBuPI0s="; // TeamSpeak Root seed
    let d_base_b64 = "6BNOfdZZvplSDRM6EUYxtkYFzpb83GO840322dgtRHM="; // Base key prv

    let d0_seed_bytes = BASE64_STANDARD.decode(d0_seed_b64).unwrap();
    let d_base_bytes = BASE64_STANDARD.decode(d_base_b64).unwrap();

    // Derive Root raw private scalar d0 by hashing and clamping
    let mut hasher = Sha512::new();
    hasher.update(&d0_seed_bytes);
    let seed_hash = hasher.finalize();
    let mut d0_scalar_bytes = [0u8; 32];
    d0_scalar_bytes.copy_from_slice(&seed_hash[0..32]);
    d0_scalar_bytes[0] &= 248;
    d0_scalar_bytes[31] &= 127;
    d0_scalar_bytes[31] |= 64;
    let d0 = Scalar::from_bytes_mod_order(d0_scalar_bytes);
    ...
    let entry2_bytes = hex::decode("00e9fb59be55ae5f9bffbd892841fe7f0e0ec0d2b4e8ac0f9b792f9767840f05ae05189b68d53eaae5690000").unwrap();
        ...
        for skip0 in &[false, true] {
            ...
            for sign0 in &[-1f64, 1f64] {
                ...
                if pk1_derived == pk0_expected.as_slice() || flipped_pk1 == pk0_expected.as_slice() {
                    println!("MATCH Level 1 (Root -> Entry 0)!!!");
