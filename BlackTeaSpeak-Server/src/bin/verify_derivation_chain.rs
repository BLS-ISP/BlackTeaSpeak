...
fn main() {
    ...
    let pk2_expected = hex::decode("7c28498b980304b795439e55d9cee696f0af5ed3af45d056b59a6f03078302a8").unwrap();
        ...
        for skip0 in &[false, true] {
            ...
            for sign0 in &[-1f64, 1f64] {
                ...
                if pk1_derived == pk0_expected.as_slice() || flipped_pk1 == pk0_expected.as_slice() {
                    println!("MATCH Level 1 (Entry 0 -> Entry 1)!!!");
                    println!("  SHA256: {}", use_sha256);
                    println!("  skip0: {}", skip0);
                    println!("  sign0: {}", sign0);
                    println!("  Derived Level 1 private key (b64): {}", BASE64_STANDARD.encode(d1.to_bytes()));
                    ...
                    for skip1 in &[false, true] {
                        ...
                        for sign1 in &[-1f64, 1f64] {
                            ...
                            if pk2_derived == pk1_expected.as_slice() || flipped_pk2 == pk1_expected.as_slice() {
                                println!("  MATCH Level 2 (Entry 1 -> Entry 2)!!!");
                                println!("    skip1: {}", skip1);
                                println!("    sign1: {}", sign1);
                                println!("    Derived Level 2 private key (b64): {}", BASE64_STANDARD.encode(d2.to_bytes()));

                                // Now let's try Level 3
                                for skip2 in &[false, true] {
                                    ...
                                    for sign2 in &[-1f64, 1f64] {
                                        ...
                                        if pk3_derived == pk2_expected.as_slice() || flipped_pk3 == pk2_expected.as_slice() {
                                            println!("    MATCH Level 3 (Entry 2 -> Server Key)!!!");
                                            println!("      skip2: {}", skip2);
                                            println!("      sign2: {}", sign2);
                                            println!("      Derived Server Private Key (b64): {}", BASE64_STANDARD.encode(d3.to_bytes()));
                                        }
                                    }
                                }
                            }
    ...
    println!("Derivation chain verification finished.");
}
