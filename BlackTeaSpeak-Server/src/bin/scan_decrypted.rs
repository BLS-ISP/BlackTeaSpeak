    ...
    for offset in 10..=150 {
        ...
        for &(seed_name, seed) in &seeds {
            // Try twisted
            {
                ...
                let decoded = decrypt_body(encrypted_body, &mut || mt.next());
                scan_buffer(&decoded, &target_bytes, "twisted", seed_name, offset);
            }
            // Try untwisted
            {
                let mut mt = MtUntwisted::new(seed);
                let decoded = decrypt_body(encrypted_body, &mut || mt.next());
                scan_buffer(&decoded, &target_bytes, "untwisted", seed_name, offset);
            }
        }
    }
...
fn decrypt_body(encrypted: &[u8], next_rand: &mut dyn FnMut() -> u64) -> Vec<u8> {
