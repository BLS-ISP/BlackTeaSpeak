import base64

key_b64 = ""
key_bytes = base64.b64decode(key_b64)

print("Total bytes:", len(key_bytes))
print("Bytes 10 to 16:", key_bytes[10:16])
print("Bytes 10 to 16 hex:", key_bytes[10:16].hex())
print("Bytes from 10 hex:", key_bytes[10:].hex())
