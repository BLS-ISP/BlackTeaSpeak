import base64
import struct

class Mt19937_64:
    def __init__(self, seed):
        self.mt = [0] * 312
        self.mt[0] = seed
        for i in range(1, 312):
            self.mt[i] = (6364136223846793005 * (self.mt[i-1] ^ (self.mt[i-1] >> 62)) + i) & 0xffffffffffffffff
        self.index = 312

    def next(self):
        if self.index >= 312:
            for i in range(312):
                y = (self.mt[i] & 0xFFFFFFFF80000000) | (self.mt[(i + 1) % 312] & 0x7FFFFFFF)
                x = y >> 1
                if y & 1 != 0:
                    x ^= 0xB5026F5AA96619E9
                self.mt[i] = self.mt[(i + 156) % 312] ^ x
            self.index = 0
        x = self.mt[self.index]
        self.index += 1
        x ^= (x >> 29) & 0x5555555555555555
        x ^= (x << 17) & 0x71D67FFFEDA60000
        x ^= (x << 37) & 0xFFF7EEE000000000
        x ^= x >> 43
        return x

class MtUntwisted:
    def __init__(self, seed):
        self.mt = [0] * 312
        self.mt[0] = seed
        for i in range(1, 312):
            self.mt[i] = (6364136223846793005 * (self.mt[i-1] ^ (self.mt[i-1] >> 62)) + i) & 0xffffffffffffffff
        self.index = 0

    def next(self):
        if self.index >= 312:
            for i in range(312):
                y = (self.mt[i] & 0xFFFFFFFF80000000) | (self.mt[(i + 1) % 312] & 0x7FFFFFFF)
                x = y >> 1
                if y & 1 != 0:
                    x ^= 0xB5026F5AA96619E9
                self.mt[i] = self.mt[(i + 156) % 312] ^ x
            self.index = 0
        x = self.mt[self.index]
        self.index += 1
        x ^= (x >> 29) & 0x5555555555555555
        x ^= (x << 17) & 0x71D67FFFEDA60000
        x ^= (x << 37) & 0xFFF7EEE000000000
        x ^= x >> 43
        return x

key_b64 = ""
key_bytes = base64.b64decode(key_b64)

print("Scanning all possible offsets in key_bytes...")
for pos in range(len(key_bytes) - 16):
    block = key_bytes[pos:]
    version = struct.unpack("<H", block[0:2])[0]
    seed_le = struct.unpack("<Q", block[2:10])[0]
    seed_be = struct.unpack(">Q", block[2:10])[0]
    verify_offset = block[10]
    verify = block[11:16]
    
    # Check if verify_offset looks reasonable (e.g. <= 250)
    if verify_offset == 0 or verify_offset > 250:
        continue
        
    for seed_name, seed in [("LE", seed_le), ("BE", seed_be)]:
        for gen_name, gen_cls in [("twisted", Mt19937_64), ("untwisted", MtUntwisted)]:
            mt = gen_cls(seed)
            for _ in range(verify_offset):
                mt.next()
            rec = mt.next()
            rec_40 = (rec ^ (rec >> 40)) & 0xFFFFFFFFFF
            rec_bytes = struct.pack("<Q", rec_40)[0:5]
            if rec_bytes == verify:
                print(f"MATCH at pos {pos}: version={version}, seed_name={seed_name}, seed={seed}, verify_offset={verify_offset}, gen={gen_name}")
