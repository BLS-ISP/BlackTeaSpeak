...
import * as url_utils from "node:url";
...
namespace generator {
    ...
    const matchesSearchPattern = (file: ProjectResource, localName: string): boolean => {
        const normalizedLocalName = localName.replace(/\\/g, "/");
        return file["search-pattern"].exec(localName) !== null || file["search-pattern"].exec(normalizedLocalName) !== null;
    };

    const isSearchExcluded = (file: ProjectResource, candidatePath: string): boolean => {
        const excludePattern = file["search-exclude"];
        return excludePattern !== undefined && excludePattern.exec(candidatePath) !== null;
    };
    ...
    function file_matches_options(file: ProjectResource, options: SearchOptions) {
        ...
        if(!file["build-target"].split("|").includes(options.mode))
            return false;

        return !(Array.isArray(file["req-parm"]) && file["req-parm"].some(e => !options.parameter.some(p => p.toLowerCase() === e.toLowerCase())));
    }
    ...
    export async function search_files(files: ProjectResource[], options: SearchOptions) : Promise<Entry[]> {
        ...
        for(const file of files) {
            ...
            for(const f of files) {
                const local_name = f.slice(normal_local.length);
                if(!matchesSearchPattern(file, local_name))
                    continue;

                if(isSearchExcluded(file, f))
                    continue;
                ...
                if(result.some(e => e.target_path === data.target_path))
                    continue;
                result.push(data);
            }
        }
...
namespace server {
    ...
    async function handle_api_request(response: http.ServerResponse, url: url_utils.URL) {
        if(url.searchParams.get("type") === "files") {
            ...
            response.end();
            return;
        } else if(url.searchParams.get("type") === "file") {
            let p = path.join(url.searchParams.get("path") || "", url.searchParams.get("name") || "").replace(/\\/g, "/");
            if(!p.startsWith("/")) p = "/" + p;
    ...
    function handleHTTPRequest(request: http.IncomingMessage, response: http.ServerResponse) {
        let url: url_utils.URL;
        try {
            url = new url_utils.URL(request.url || "/", "http://localhost");
        } catch(error) {
...
namespace watcher {
    ...
    export abstract class Watcher {
        ...
        private _callback_init_fail: (error: Error) => any;
        ...
        private handle_exit(code: number | null, signal: string | null) {
            ...
            if(this._callback_init_fail)
                this._callback_init_fail(new Error("unexpected exit with code " + code));
        }
...
async function git_tag() {
    ...
    if(git_rev.includes("/"))
        return fs.readFileSync(path.join(__dirname, ".git", git_rev.slice(5).trim())).toString().slice(0, 7);
    else
        return git_rev.slice(0, 7);
}
...
async function main_generate(target: "client" | "web", mode: "rel" | "dev", dest_path: string, args: any[]) {
    ...
    if(await fs.pathExists(dest_path))
    ...
    if(os.platform() === "win32") {
        ...
        linker = async (source, target) => {
            ...
            if(stderr)
                throw new Error("failed to create link: " + stderr);
        }
    }
...
async function main(args: string[]) {
    if(args.length >= 1) {
        if((args[0].toLowerCase() === "develop" && args.length >= 2) || args[0].toLowerCase() === "ndevelop") {
            ...
            await main_develop(is_node, target, pargs.length > 1 ? Number.parseInt(pargs[1]) : 8081, switches);
            return;
        }
    }
    if(args.length >= 2) {
