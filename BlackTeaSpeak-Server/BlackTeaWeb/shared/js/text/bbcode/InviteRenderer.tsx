...
const kLegacyInviteUrlRegex = /^(https:\/\/)?(teaspeak.de\/|join.teaspeak.de\/(invite\/)?)([a-zA-Z0-9]{4})$/i;

function extractInviteLinkId(url: string) {
    const legacyMatch = kLegacyInviteUrlRegex.exec(url);
    if(legacyMatch) {
        return legacyMatch[4];
    }

    const parsedUrl = document.createElement("a");
    parsedUrl.href = url;

    const inviteReference = new URLSearchParams(parsedUrl.search).get(AppParameters.KEY_CONNECT_INVITE_REFERENCE.key);
    const parsedOrigin = parsedUrl.origin || `${parsedUrl.protocol}//${parsedUrl.host}`;
    if(parsedOrigin === window.location.origin && /^[a-zA-Z0-9]{4}$/.test(inviteReference || "")) {
        return inviteReference;
    }
}

export function isInviteLink(url: string) : boolean {
    return extractInviteLinkId(url) !== undefined;
}
...
const useInviteLink = (linkId: string): LocalInviteInfo => {
    ...
    useEffect(() => {
        ...
        return () => {
            localInviteCallbacks[linkId]?.remove(callback);
        };
    }, [linkId]);
...
const InviteLinkContent = (props: { inviteLinkId: string, url: string }) => {
    const linkInfo = useInviteLink(props.inviteLinkId);
...
export const InviteLinkRenderer = (props: { url: string, handlerId: string }) => {
    const inviteLinkId = extractInviteLinkId(props.url);
    if(!inviteLinkId) {
        return <SimpleUrlRenderer target={props.url}>{props.url}</SimpleUrlRenderer>;
    }

    return <InviteLinkContent inviteLinkId={inviteLinkId} url={props.url} />;
}
