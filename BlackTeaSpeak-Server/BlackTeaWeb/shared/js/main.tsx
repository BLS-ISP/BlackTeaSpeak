...
import type {ActionResult} from "tc-services";
...
async function doHandleConnectRequest(serverAddress: string, serverUniqueId: string | undefined, parameters: UrlParameterParser) : Promise<ConnectRequestResult> {
    ...
    if(!getAudioBackend().isInitialized()) {
        const autoConnectAddress = parameters.getValue(AppParameters.KEY_CONNECT_ADDRESS, undefined);
        if(autoConnectAddress) {
            logWarn(LogCategory.GENERAL, tr("Skipping audio initialization prompt for URL-based auto connect to %s."), serverAddress);
        } else {
...
function createConnectRequestUrl(properties: ConnectRequestData) {
    const urlBuilder = new UrlParameterBuilder();
    urlBuilder.setValue(AppParameters.KEY_CONNECT_ADDRESS, properties.address);
    ...
    urlBuilder.setValue(AppParameters.KEY_CONNECT_TOKEN, properties.token);
    urlBuilder.setValue(AppParameters.KEY_CONNECT_CHANNEL, properties.channel);
    urlBuilder.setValue(AppParameters.KEY_CONNECT_CHANNEL_PASSWORD, properties.channelPassword);
    urlBuilder.setValue(AppParameters.KEY_CONNECT_INVITE_REFERENCE, properties.inviteReference);

    urlBuilder.setValue(AppParameters.KEY_CONNECT_SERVER_PASSWORD, properties.serverPassword);
    if(typeof properties.serverPassword === "string" || typeof properties.channelPassword === "string") {
        urlBuilder.setValue(AppParameters.KEY_CONNECT_PASSWORDS_HASHED, properties.passwordsHashed || false);
    }

    return new URL(`https://localhost/?${urlBuilder.build()}`);
}

function createConnectRequestData(parameters: UrlParameterParser, address: string): ConnectRequestData {
    const serverPassword = parameters.getValue(AppParameters.KEY_CONNECT_SERVER_PASSWORD, undefined);
    const channelPassword = parameters.getValue(AppParameters.KEY_CONNECT_CHANNEL_PASSWORD, undefined);
    const passwordsHashed =
        typeof serverPassword === "string" || typeof channelPassword === "string"
            ? parameters.getValue(AppParameters.KEY_CONNECT_PASSWORDS_HASHED)
            : undefined;

    return {
        address: address,
        profile: parameters.getValue(AppParameters.KEY_CONNECT_PROFILE, undefined),
        username: parameters.getValue(AppParameters.KEY_CONNECT_NICKNAME, undefined),
        token: parameters.getValue(AppParameters.KEY_CONNECT_TOKEN, undefined),
        channel: parameters.getValue(AppParameters.KEY_CONNECT_CHANNEL, undefined),
        inviteReference: parameters.getValue(AppParameters.KEY_CONNECT_INVITE_REFERENCE, undefined),
        serverPassword: serverPassword,
        channelPassword: channelPassword,
        passwordsHashed: passwordsHashed,
    };
}
...
export function handle_connect_request(properties: ConnectRequestData, _connection: ConnectionHandler) {
    const url = createConnectRequestUrl(properties);
...
const task_connect_handler: loader.Task = {
    ...
    function: async () => {
        ...
        const connectData = createConnectRequestData(AppParameters.Instance, address);
            ...
            try {
                await chandler.post_connect_request(connectData, async () => {
                    return await promptYesNo({
                        title: tr("Another BlackTeaSpeak Web instance is already running"),
                        question: tra("Another BlackTeaSpeak Web instance is already running.\nWould you like to connect there?")
                    });
                });
                ...
                createInfoModal(
                    ...
                    formatMessage(tr("You're connecting to {0} within the other BlackTeaSpeak Web instance.\nYou could now close this page."), connectData.address),
                    {
                        closeable: false,
                        footer: undefined
                    }
                ...
                logInfo(LogCategory.CLIENT, tr("Failed to execute connect within other BlackTeaSpeak Web instance. Using this one. Error: %o"), error);
            }
