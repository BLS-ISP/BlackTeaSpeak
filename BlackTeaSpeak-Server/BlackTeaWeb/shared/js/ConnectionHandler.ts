    ...
    async startVoiceRecorder(notifyError: boolean) : Promise<{ state: "success" | "no-input" } | { state: "error", message: string }> {
        ...
        if(input.currentState() === InputState.PAUSED && this.connection_state === ConnectionState.CONNECTED) {
            try {
                ...
                let errorMessage;
                let showErrorModal = notifyError;
                if(error === InputStartError.ENOTSUPPORTED) {
                    ...
                    errorMessage = tr("Microphone access was denied");
                    showErrorModal = false;
                } else if(error === InputStartError.ESYSTEMDENIED) {
                    errorMessage = tr("Microphone access is blocked by the browser or system");
                    showErrorModal = false;
                ...
                this.log.log("error.custom", { message: tra("Microphone start failed. {}", errorMessage) });
                if(showErrorModal) {
