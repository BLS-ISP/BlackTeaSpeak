...
export class PermissionManager extends AbstractCommandHandler {
    ...
    /* Static info mapping until BlackTeaSpeak implements a detailed info */
    ...
    private onNeededPermissions(json: any[]) {
        if(this.permissionList.length == 0) {
            logTrace(LogCategory.PERMISSIONS, tr("Got needed permissions before the permission list arrived. Caching the update until the permission list is available."));
