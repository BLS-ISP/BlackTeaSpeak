...
export class GroupManager extends AbstractCommandHandler {
    ...
    request_permissions(group: Group) : Promise<PermissionValue[]> {
        for(let request of this.groupPermissionRequests)
            if(request.group_id == group.id && request.group_target == group.target && request.promise.time() + 1000 > Date.now())
                return request.promise;
        let req = new GroupPermissionRequest();
        req.group_id = group.id;
        req.group_target = group.target;
        req.promise = new LaterPromise<PermissionValue[]>();
        this.groupPermissionRequests.push(req);

        const command = group.target == GroupTarget.SERVER ? "servergrouppermlist" : "channelgrouppermlist";
        const payload = group.target == GroupTarget.SERVER ? { sgid: group.id } : { cgid: group.id };

        this.connectionHandler.serverConnection.send_command(command, payload).catch(error => {
    ...
    private handleGroupPermissionList(json: any[]) {
        ...
        for(let req of this.groupPermissionRequests)
            if(req.group_id == group.id && req.group_target == group.target)
                requests.push(req);
