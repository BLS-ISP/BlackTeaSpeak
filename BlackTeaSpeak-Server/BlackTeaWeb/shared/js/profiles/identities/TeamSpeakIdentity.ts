...
export namespace CryptoHelper {
    ...
    export async function decryptBlackTeaSpeakIdentity(buffer: Uint8Array) : Promise<string> {
    ...
    export async function encryptBlackTeaSpeakIdentity(buffer: Uint8Array) : Promise<string> {
        const length = Math.min(buffer.length, 100);
        for(let i = 0; i < length; i++)
            buffer[i] ^= kCryptKey.charCodeAt(i);
...
export class BlackTeaSpeakHandshakeHandler extends AbstractHandshakeIdentityHandler {
    identity: BlackTeaSpeakIdentity;
    handler: HandshakeCommandHandler<BlackTeaSpeakHandshakeHandler>;

    constructor(connection: AbstractServerConnection, identity: BlackTeaSpeakIdentity) {
        super(connection);
...
export class BlackTeaSpeakIdentity implements Identity {
    static async generateNew() : Promise<BlackTeaSpeakIdentity> {
        ...
        const identity = new BlackTeaSpeakIdentity(private_key, "0", undefined, false);
        await identity.initialize();
        return identity;
    }

    static async import_ts(ts_string: string, ini?: boolean) : Promise<BlackTeaSpeakIdentity> {
        ...
        const key64 = await CryptoHelper.decryptBlackTeaSpeakIdentity(buffer);

        const identity = new BlackTeaSpeakIdentity(key64, hash, name, false);
        await identity.initialize();
        return identity;
    }
    ...
    constructor(private_key?: string, hash?: string, name?: string, initialize?: boolean) {
        ...
        if(this.private_key && (typeof(initialize) === "undefined" || initialize)) {
            this.initialize().catch(error => {
                logError(LogCategory.IDENTITIES, "Failed to initialize BlackTeaSpeakIdentity (%s)", error);
    ...
    async decode(data: string) : Promise<void> {
        ...
        if(json.version == 2) {
            ...
            const clone = await BlackTeaSpeakIdentity.import_ts(key, false);
    ...
    async level() : Promise<number> {
        ...
        return BlackTeaSpeakIdentity.calculateLevel(hash);
    }
    ...
    async improveLevelNative(target: number, threads: number, active_callback: () => boolean, callback_level?: (current: number) => any, callback_status?: (hash_rate: number) => any) : Promise<Boolean> {
        ...
        const next_hash = () => {
            ...
            if(current_hash.length < iterations.toString().length) {
                current_hash = BlackTeaSpeakIdentity.string_add(iterations.toString(), current_hash);
            } else {
                current_hash = BlackTeaSpeakIdentity.string_add(current_hash, iterations.toString());
            }
            return current_hash;
        };
    ...
    async improveLevelJavascript(target: number, activeCallback: () => boolean) : Promise<number> {
        ...
        while(currentLevel < target) {
            ...
            incrementCounter();
            const newLevel = await BlackTeaSpeakIdentity.calculateLevel(new Uint8Array(await crypto.subtle.digest("SHA-1", bufferView)));
    ...
    async export_ts(ini?: boolean) : Promise<string> {
        ...
        const identity = this.hash_number + "V" + await CryptoHelper.encryptBlackTeaSpeakIdentity(new Uint8Array(str2ab8(this.private_key)));
        if(!ini) return identity;

        return "[Identity]\n" +
                "id=BlackTeaWeb-Exported\n" +
                "identity=\"" + identity + "\"\n" +
    ...
    spawn_identity_handshake_handler(connection: AbstractServerConnection): HandshakeIdentityHandler {
        return new BlackTeaSpeakHandshakeHandler(connection, this);
    }
}