...
function getForumApiURL() {
    const configuredUrl = settings.getValue(Settings.KEY_TEAFORO_URL);
    if(__build.target === "web" && configuredUrl === "https://forum.blackteaspeak.de/") {
        return "./";
    }
...
export async function login(username: string, password: string, captcha?: any) : Promise<LoginResult> {
    ...
    try {
        ...
        await adapter.set("blackteaspeak-forum-data", response["data"]);
        await adapter.set("blackteaspeak-forum-sign", response["sign"]);
        await adapter.set("blackteaspeak-forum-auth", response["auth-key"]);
...
async function resetForumLocalData() {
    const adapter = getStorageAdapter();
    await adapter.delete("blackteaspeak-forum-data");
    await adapter.delete("blackteaspeak-forum-sign");
    await adapter.delete("blackteaspeak-forum-auth");
}
...
export async function renew_data() : Promise<"success" | "login-required"> {
    ...
    try {
        ...
        await getStorageAdapter().set("blackteaspeak-forum-data", response["data"]);
        await getStorageAdapter().set("blackteaspeak-forum-sign", response["sign"]);
        fidentity.update_forum();
    } catch(error) {
        logError(LogCategory.GENERAL, tr("Failed to parse forum given data: %o"), error);
...
loader.register_task(loader.Stage.JAVASCRIPT_INITIALIZING, {
    ...
    priority: 10,
    function: async () => {
        const rawData = await getStorageAdapter().get("blackteaspeak-forum-data");
        const rawSign = await getStorageAdapter().get("blackteaspeak-forum-sign");
        const rawForumAuth = await getStorageAdapter().get("blackteaspeak-forum-auth");
