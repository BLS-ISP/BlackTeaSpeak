...
import {spawnWhisperTargetsModal} from "tc-shared/ui/modal/whisper-targets/Controller";
...
export function initialize(event_registry: Registry<ClientGlobalControlEvents>) {
    ...
    event_registry.on("action_open_window", event => {
        ...
        switch (event.window) {
            ...
            case "server-whisper-targets":
                const whisperConnection = event.connection || server_connections.getActiveConnectionHandler();
                if(!whisperConnection || !whisperConnection.connected) {
                    createErrorModal(tr("You have to be connected"), tr("You have to be connected!")).open();
                    return;
                }

                spawnWhisperTargetsModal(whisperConnection);
                break;
