        ...
        if(!this.broadcastIds[type]) {
            ...
            switch (rtpState) {
                ...
                case RemoteRTPTrackState.Unbound:
                    logTrace(LogCategory.VIDEO, tr("Updated video broadcast state with an unbound remote video track. Treating this as a normal stopped state."));
                    this.setBroadcastState(type, VideoBroadcastState.Stopped);
                    break;

                case RemoteRTPTrackState.Destroyed:
                    logTrace(LogCategory.VIDEO, tr("Updated video broadcast state with a destroyed remote video track. Treating this as a normal stopped state."));
                    this.setBroadcastState(type, VideoBroadcastState.Stopped);
                    break;
