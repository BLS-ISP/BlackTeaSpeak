...
export class SdpProcessor {
    private static readonly kAudioCodecs: SdpCodec[] = [
        ...
        // If not, Firefox start to make trouble and isn't replaying the sound...
        {
            ...
            rate: 48000,
            encoding: 2,
            fmtp: { minptime: 1, maxptime: 20, useinbandfec: 1, usedtx: 1, stereo: 0, "sprop-stereo": 0, maxaveragebitrate: 128000 },
        ...
        },
        {
            ...
            rate: 48000,
            encoding: 2,
            fmtp: { minptime: 1, maxptime: 20, useinbandfec: 1, usedtx: 1, stereo: 1, "sprop-stereo": 1, maxaveragebitrate: 510000 },
            rtcpFb: [ "transport-cc", "nack", "goog-remb" ]
        },
    ];
        ...
        for(const media of sdp.media) {
            ...
            for(const group of media.ssrcGroups.slice()) {
                if(group.semantics === "FID") {
                    ...
                    media.ssrcs = media.ssrcs.filter(ssrc => ids.indexOf(Number.parseInt(String(ssrc.id), 10) >>> 0) === -1);
