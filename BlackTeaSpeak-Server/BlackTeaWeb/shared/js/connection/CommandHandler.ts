...
export class ConnectionCommandHandler extends AbstractCommandHandler {
    ...
    handleCommandClientEnterView(json) {
        ...
        const localClient = this.connection.client.getClient();
        ...
        for(const entry of json) {
            ...
            const isLocal = localClient && parseInt(entry["clid"]) === localClient.clientId();
            const isCurrentChannel = localClient && channel === localClient.currentChannel();
    ...
    handleNotifyTextMessage(json) {
        ...
        if(mode == 1){
            ...
            const messageTimestamp = parseInt(json["timestamp"]);
            ...
            conversation.handleIncomingMessage(chatPartner, !targetIsOwn, {
                ...
                timestamp: isNaN(messageTimestamp) ? Date.now() : messageTimestamp,
                message: json["msg"]
            });
            if(targetIsOwn) {
    ...
    handleNotifyChannelUnsubscribed(json) {
        const localClient = this.connection.client.getClient();
        const currentChannel = localClient ? localClient.currentChannel() : undefined;
