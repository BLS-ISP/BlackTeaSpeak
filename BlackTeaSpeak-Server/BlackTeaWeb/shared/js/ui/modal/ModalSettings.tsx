...
import {BlackTeaSpeakIdentity} from "tc-shared/profiles/identities/TeamSpeakIdentity";
...
import * as forum from "tc-shared/profiles/identities/blackteaspeak-forum";
...
export namespace modal_settings {
    ...
    export function initialize_identity_profiles_controller(event_registry: Registry<SettingProfileEvents>) {
        ...
        const build_profile_info = (profile: ConnectionProfile) => {
            ...
            const teamspeak_data = profile.selectedIdentity(IdentitifyType.TEAMSPEAK) as BlackTeaSpeakIdentity;
        ...
        event_registry.on("query-identity-teamspeak", event => {
            ...
            const ts = profile.selectedIdentity(IdentitifyType.TEAMSPEAK) as BlackTeaSpeakIdentity;
            if (!ts) {
        ...
        event_registry.on("generate-identity-teamspeak", event => {
            ...
            BlackTeaSpeakIdentity.generateNew().then(identity => {
        ...
        event_registry.on("improve-identity-teamspeak-level", event => {
            ...
            const identity = profile.selectedIdentity(IdentitifyType.TEAMSPEAK) as BlackTeaSpeakIdentity;
            if (!identity) return;
        ...
        /* profile info */
        {
            ...
            /* general info */
            {
                ...
                /* nickname name */
                {
                    ...
                    const update_name = () => input.prop("disabled", false)
                        .val(last_name)
                        .attr("placeholder", fallback_names[current_identity_type] || tr("Another BlackTeaSpeak user"))
                        .parent().removeClass("is-invalid");
