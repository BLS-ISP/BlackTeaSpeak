...
const LinkRow = React.memo((props: { title: React.ReactNode, value: React.ReactNode, className: string, copyValue?: string, disabled?: boolean }) => {
    return (
        <div className={cssStyle.linkRow}>
            <div className={cssStyle.title}>{props.title}</div>
            <div className={cssStyle.output + " " + props.className}>
                <span>{props.value}</span>
                <ButtonCopy disabled={props.disabled || !props.copyValue} onCopy={() => {
                    if(props.copyValue) {
...
const Link = React.memo(() => {
    ...
    return (
        <div className={cssStyle.containerLink}>
            <LinkRow
                title={<Translatable>Invite link</Translatable>}
                value={value}
                className={className}
                copyValue={copyValue}
                disabled={link.status === "generating"}
            />
            {link.nativeUrl ? (
                <LinkRow
                    title={<Translatable>TeaClient link</Translatable>}
                    value={link.nativeUrl}
                    className={cssStyle.success}
                    copyValue={link.nativeUrl}
                />
            ) : undefined}
