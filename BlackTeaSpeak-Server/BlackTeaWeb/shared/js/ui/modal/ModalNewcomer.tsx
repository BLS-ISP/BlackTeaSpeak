...
import * as ReactDOM from "../ReactDOMCompat";
...
export function openModalNewcomer(): Modal {
    let modal = createModal({
        header: tra("{} quick setup", __build.target === "web" ? "BlackTeaSpeak Web" : "TeaClient"),
        body: () => $("#tmpl_newcomer").renderTag({
            is_web: __build.target === "web"
        }).children(),
        footer: null,
    ...
    modal.htmlTag.addClass("modal-newcomer-shell");
...
function initializeBasicFunctionality(tag: JQuery, event_registry: Registry<EventModalNewcomer>) {
    ...
    const step_order: Array<EventModalNewcomer["show_step"]["step"]> = ["welcome", "microphone", "identity", "finish"];
    ...
    event_registry.on("show_step", event => {
        ...
        const current_step_index = step_order.indexOf(event.step);
        container_header.find(".step-header").each((_, element) => {
            const header_step = $(element);
            const header_step_name = header_step.data("step");
            const header_step_index = step_order.indexOf(header_step_name);

            header_step.removeClass("is-active is-done is-upcoming");
            if (header_step_index >= 0 && header_step_index < current_step_index) {
                header_step.addClass("is-done");
            } else if (header_step_index === current_step_index) {
                header_step.addClass("is-active");
            } else {
                header_step.addClass("is-upcoming");
            }
        });
    });
    ...
    /* button controller */
    {
        ...
        event_registry.on("show_step", event => {
            ...
            button_next_step.text(next_step[current_step] ? tr("Continue") : tr("Open client"));
            button_last_step.text(last_step[current_step] ? tr("Back") : tr("Skip for now"));
        });
    ...
    /* the help sequence */
    {
        ...
        const show_profile_settings_help = () => {
            ...
            set_help_text(tr( /* @tr-ignore */
                ...
                "BlackTeaSpeak Web currently supports two ways to identify yourself:\n" +
                "1. Identify yourself by your forum account\n" +
...
function initializeStepMicrophone(tag: JQuery, event_registry: Registry<EventModalNewcomer>, modal: Modal) {
    let helpStep = 0;
    let stepShown = false;
    const render_target = tag.find(".step-host")[0] || tag[0];
    ...
    ReactDOM.render(<MicrophoneSettings events={settingEvents}/>, render_target);

    modal.close_listener.push(() => {
        settingEvents.fire("notify_destroy");
        ReactDOM.unmountComponentAtNode(render_target);
    });
