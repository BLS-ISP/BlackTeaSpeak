...
const PlayerController = React.memo((props: { events: Registry<VideoViewerEvents> }) => {
    ...
    const syncProgress = () => {
        const loadedSeconds = getLoadedSeconds();
        const playedSeconds = getCurrentTime();

        kLogPlayerEvents && logTrace(LogCategory.GENERAL, tr("ReactPlayer::onProgress %d seconds played, %d seconds buffered. Player state: %s"), playedSeconds, loadedSeconds, playerState.current);

        currentTime.current = { buffer: loadedSeconds, play: playedSeconds };
        ...
        props.events.fire("notify_local_status", {
            status: {
                status: "playing",
                timestampBuffer: Math.floor(loadedSeconds),
                timestampPlay: Math.floor(playedSeconds)
            }
        });
    };
    ...
    props.events.reactUse("notify_following_status", event => {
        ...
        if(event.status.status === "playing" && player.current) {
            const distance = Math.abs(getCurrentTime() - event.status.timestampPlay);
            ...
            if(doSeek) {
                seekTo(event.status.timestampPlay);
            }
    ...
    return (
        <ReactPlayer
            ...
            src={videoUrl === "querying" ? undefined : videoUrl}
            height={"100%"}
            width={"100%"}

            onError={event => logTrace(LogCategory.GENERAL, "onError(%o)", event)}
            onWaiting={() => {
            ...
            onDurationChange={() => {
                kLogPlayerEvents && logTrace(LogCategory.GENERAL, tr("ReactPlayer::onDuration(%d)"), player.current?.duration || 0);
            }}
            ...
            onSeeked={() => {
                kLogPlayerEvents && logTrace(LogCategory.GENERAL, tr("ReactPlayer::onSeek(%d)"), getCurrentTime());
            }}
