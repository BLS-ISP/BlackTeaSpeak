...
export function initialize_audio_microphone_controller(events: Registry<MicrophoneSettingsEvents>) {
    ...
    /* TODO: Only do this on user request? */
    {
        ...
        let originalHandlerId: string | undefined;
        if(oldOwner instanceof ConnectionRecorderProfileOwner) {
            try {
                originalHandlerId = oldOwner.getConnection()?.handlerId;
            } catch (error) {
                logWarn(LogCategory.AUDIO, tr("Failed to resolve previous recorder owner connection: %o"), error);
            }
        }
