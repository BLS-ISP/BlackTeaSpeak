...
const kPreferredDefaultMaxFrameRate = 60;
const kEstimatedBitsPerPixel = 3.9;
...
function updateBroadcastConfigFromSource(source: VideoSource, constraints: VideoBroadcastConfig) {
    ...
    if(!constraints.maxFrameRate || constraints.maxFrameRate < 1) {
        constraints.maxFrameRate = trackSettings.frameRate || kPreferredDefaultMaxFrameRate;
    }
}

function resolveDefaultMaxFrameRate(source: VideoSource) : number {
    const maxFrameRate = source.getCapabilities().maxFrameRate;
    if(typeof maxFrameRate === "number" && maxFrameRate > 0) {
        return Math.min(kPreferredDefaultMaxFrameRate, maxFrameRate);
    }

    return kPreferredDefaultMaxFrameRate;
}

function estimateTargetBitrate(width: number, height: number, frameRate: number) : number {
    return Math.ceil(kEstimatedBitsPerPixel * width * height * (frameRate / 30));
}

function resolveDefaultMaxBandwidth(type: VideoBroadcastType, constraints: VideoBroadcastConfig) : number {
    const defaultMaxBandwidth = settings.getValue(Settings.KEY_VIDEO_DEFAULT_MAX_BANDWIDTH);
    if(type !== "screen" || defaultMaxBandwidth === 0) {
        return defaultMaxBandwidth;
    }

    return Math.max(defaultMaxBandwidth, estimateTargetBitrate(constraints.width, constraints.height, constraints.maxFrameRate));
}

async function generateAndApplyDefaultConfig(source: VideoSource, type: VideoBroadcastType) : Promise<VideoBroadcastConfig> {
    const videoTrack = source.getStream().getVideoTracks()[0];
    ...
    const broadcastConstraints: VideoBroadcastConfig = {
        ...
        maxFrameRate: resolveDefaultMaxFrameRate(source)
    ...
    broadcastConstraints.maxBandwidth = resolveDefaultMaxBandwidth(type, broadcastConstraints);

    broadcastConstraints.dynamicQuality = settings.getValue(Settings.KEY_VIDEO_DYNAMIC_QUALITY);
    broadcastConstraints.dynamicFrameRate = type === "screen" ? false : settings.getValue(Settings.KEY_VIDEO_DYNAMIC_FRAME_RATE);
...
class VideoSourceController {
    ...
    async setCurrentSource(source: VideoSource | string | undefined) {
        ...
        if(typeof source === "object") {
            ...
            if(!this.currentConstraints) {
                this.currentConstraints = await generateAndApplyDefaultConfig(source, this.type);
            }
        }
