...
class GroupsList extends React.PureComponent<{ events: Registry<PermissionModalEvents>, target: "server" | "channel" }, {
    ...
    private handleGroupTabActive(events: PermissionModalEvents["action_activate_tab"]) {
        ...
        if (events.tab === "groups-server" || events.tab === "groups-channel") {
            if (events.activeGroupId !== undefined)
                this.setState({selectedGroupId: events.activeGroupId});

            const activeGroupId = events.activeGroupId !== undefined
                ? events.activeGroupId
                : this.state.selectedGroupId > 0
                    ? this.state.selectedGroupId
                    : this.visibleGroups[0]?.id || 0;

            if (this.isListActive && activeGroupId > 0 && activeGroupId !== this.state.selectedGroupId)
                this.setState({selectedGroupId: activeGroupId});

            if (this.isListActive)
                this.props.events.fire("action_set_permission_editor_subject", {
                    mode: events.tab,
                    groupId: activeGroupId > 0 ? activeGroupId : 0,
                    clientDatabaseId: 0,
                    channelId: 0
                });
        }
...
class ChannelList extends React.Component<{ serverInfo: PermissionEditorServerInfo, events: Registry<PermissionModalEvents>, tabTarget: "channel" | "client-channel" }, { selectedChanelId: number }> {
    ...
    private handleQueryChannelsResult(event: PermissionModalEvents["notify_channels"]) {
        ...
        if (this.channels.length > 0 && this.channels.findIndex(e => e.id === this.state.selectedChanelId) === -1) {
            ...
            this.setState({selectedChanelId: channelId});
    ...
    private handleActionTabSelect(event: PermissionModalEvents["action_activate_tab"]) {
        ...
        const activeChannelId = typeof event.activeChannelId === "number"
            ? event.activeChannelId
            : this.state.selectedChanelId > 0
                ? this.state.selectedChanelId
                : this.channels[0]?.id || 0;

        if (activeChannelId > 0 && activeChannelId !== this.state.selectedChanelId)
            this.setState({ selectedChanelId: activeChannelId });
...
const ClientSelect = React.memo((props: { tabTarget: "client" | "client-channel" }) => {
    ...
    const parseClientDatabaseId = (value: string) => {
        if (!value.match(/^[0-9]+$/)) {
            return undefined;
        }

        const clientDatabaseId = parseInt(value, 10);
        return Number.isSafeInteger(clientDatabaseId) ? clientDatabaseId : undefined;
    };
    ...
    return (
        <div className={cssStyle.clientSelect}>
            <FlatInputField
                ...
                finishOnEnter={true}
                onInput={value => {
                    if (parseClientDatabaseId(value) !== undefined) {
                        refInput.current?.setState({isInvalid: false});
                    } else if (value.match(/^[0-9]+$/)) {
                        refInput.current?.setState({isInvalid: true, invalidMessage: tr("Invalid client database ID")});
                ...
                onBlur={() => {
                    ...
                    let client;
                    const clientDatabaseId = parseClientDatabaseId(value);
                    if (clientDatabaseId !== undefined) {
                        client = clientDatabaseId;
                    } else if (value.match(/^[0-9]+$/)) {
                        refInput.current?.setState({
                            isInvalid: true,
                            invalidMessage: tr("Invalid client database ID")
                        });
                        return;
                    } else if (value.length === 0) {
                        client = undefined;
