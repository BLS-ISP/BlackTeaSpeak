...
function initializePermissionEditor(connection: ConnectionHandler, modalEvents: Registry<PermissionModalEvents>, events: Registry<PermissionEditorEvents>) {
    ...
    modalEvents.on("action_set_permission_editor_subject", event => {
        ...
        switch (mode) {
            ...
            case "groups-server":
            case "groups-channel":
                editorMode = groupId > 0 ? "normal" : "unset";
                break;

            case "channel":
                editorMode = channelId > 0 ? "normal" : "unset";
                break;

            case "client":
                editorMode = clientDatabaseId > 0 ? "normal" : "unset";
                break;

            case "client-channel":
                editorMode = clientDatabaseId > 0 && channelId > 0 ? "normal" : "unset";
                break;
        }
