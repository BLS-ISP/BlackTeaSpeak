...
import TeaCupAnimatedImage from "./BlackTeaSpeakCupAnimated.png";
...
const InfoTitle = React.memo(() => {
    ...
    const title = __build.target === "web" ? "BlackTeaSpeak Web" : "TeaClient";

    return (
        <h1>
            {title} build {uiVersion}
        </h1>
    );
});
...
const ModalTitle = React.memo(() => {
    ...
    if(eggShown) {
        ...
        return <Translatable key={"web"}>About BlackTeaSpeak Web</Translatable>;
    } else {
    ...
    if(__build.target === "web") {
        result.push(
            <div className={cssStyle.version} key={"web"}>
                <div className={cssStyle.key}><Translatable>BlackTeaSpeak Web</Translatable></div>:
...
const LicenseInfo = React.memo(() => {
    let applicationName;
    if(__build.target === "web") {
        applicationName = "BlackTeaSpeak Web";
    } else {
    ...
    return (
        <p>
            ...
            Project-local license documentation is currently being consolidated.
        </p>
    )
});
                ...
                <VariablesContext.Provider value={this.variables}>
                    <div className={joinClassList(cssStyle.container, this.properties.windowed && cssStyle.windowed)}>
                        <div className={cssStyle.containerLeft}>
                            <div>
                                <img src={TeaCupAnimatedImage} alt={useTr("BlackTeaSpeak - Logo")} draggable={false} />
                            </div>
                            <div>
                                Original client work by <br/>
                                <MarkusHadenfeldt />
                            </div>
                            <div>
                                <VersionInfo/>
                        ...
                        <div className={cssStyle.containerRight}>
                            ...
                            <p>
                                <Translatable>Maintainer</Translatable>: BlackTeaSpeak<br />
                                <Translatable>Support</Translatable>: <Translatable>Handled by the current BlackTeaSpeak maintainers.</Translatable>
                            </p>
