...
export class ChannelTreeView extends ReactComponentBase<ChannelTreeViewProperties, ChannelTreeViewState> {
    ...
    scrollEntryInView(entryId: number, callback?: () => void, retryCount: number = 0) {
        const index = this.state.tree.findIndex(e => e.entryId === entryId);
        if (index === -1) {
            const providerHasEntry = this.props.dataProvider.getTreeEntries().some(entry => entry.entryId === entryId);
            if (providerHasEntry && retryCount < 3) {
                requestAnimationFrame(() => this.scrollEntryInView(entryId, callback, retryCount + 1));
                return;
            }
            if (callback) {
                callback();
            ...
            if(providerHasEntry) {
                ...
                logTrace(LogCategory.GENERAL, tr("Skipping tree scroll request because the entry is no longer present in the tree view. EntryId: %d"), entryId);
            }
            return;
        }
