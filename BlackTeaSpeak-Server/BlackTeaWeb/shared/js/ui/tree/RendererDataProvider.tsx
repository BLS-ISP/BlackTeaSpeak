...
export class RDPChannelTree {
    ...
    initialize() {
        ...
        events.push(this.events.on("notify_channel_info", event => {
            const entry = this.treeEntries[event.treeEntryId];
            if(!entry) return;
            if(!(entry instanceof RDPChannel)) {
                logError(LogCategory.CHANNEL, tr("Received channel info notify for non-channel tree entry %o."), event.treeEntryId);
                return;
            }
        ...
        events.push(this.events.on("notify_channel_icon", event => {
            ...
            if(!entry) return;
            if(!(entry instanceof RDPChannel)) {
                logError(LogCategory.CHANNEL, tr("Received channel icon notify for non-channel tree entry %o."), event.treeEntryId);
                return;
            }
        ...
        events.push(this.events.on("notify_channel_icons", event => {
            ...
            if(!entry) return;
            if(!(entry instanceof RDPChannel)) {
                logError(LogCategory.CHANNEL, tr("Received channel icons notify for non-channel tree entry %o."), event.treeEntryId);
                return;
            }
        ...
        events.push(this.events.on("notify_client_status", event => {
            ...
            if(!entry) return;
            if(!(entry instanceof RDPClient)) {
                logError(LogCategory.CHANNEL, tr("Received client status notify for non-client tree entry %o."), event.treeEntryId);
                return;
            }
        ...
        events.push(this.events.on("notify_client_name", event => {
            ...
            if(!entry) return;
            if(!(entry instanceof RDPClient)) {
                logError(LogCategory.CHANNEL, tr("Received client name notify for non-client tree entry %o."), event.treeEntryId);
                return;
            }
        ...
        events.push(this.events.on("notify_client_icons", event => {
            ...
            if(!entry) return;
            if(!(entry instanceof RDPClient)) {
                logError(LogCategory.CHANNEL, tr("Received client icons notify for non-client tree entry %o."), event.treeEntryId);
                return;
            }
        ...
        events.push(this.events.on("notify_client_talk_status", event => {
            ...
            if(!entry) return;
            if(!(entry instanceof RDPClient)) {
                logError(LogCategory.CHANNEL, tr("Received client talk notify for non-client tree entry %o."), event.treeEntryId);
                return;
            }
        ...
        events.push(this.events.on("notify_client_name_edit", event => {
            ...
            if(!entry) return;
            if(!(entry instanceof RDPClient)) {
                logError(LogCategory.CHANNEL, tr("Received client name edit notify for non-client tree entry %o."), event.treeEntryId);
                return;
            }
        ...
        events.push(this.events.on("notify_server_state", event => {
            ...
            if(!(entry instanceof RDPServer)) {
                logError(LogCategory.CHANNEL, tr("Received server state notify for non-server tree entry %o."), event.treeEntryId);
                return;
            }
