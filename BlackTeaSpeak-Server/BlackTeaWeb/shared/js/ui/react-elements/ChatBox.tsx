...
const LastUsedEmoji = () => {
    ...
    const emojiData = (emojiMartData as any).emojis;
    const lastEmoji = (emojiData?.[settingValue] || emojiData?.joy) as { skins?: { native?: string }[] } | undefined;
    const lastEmojiNative = lastEmoji?.skins?.[0]?.native;
    if(!lastEmojiNative) {
        return <img key={"fallback"} alt={""} src={"img/smiley-smile.svg"} />;
    }

    return (
        <img draggable={false} src={"https://twemoji.maxcdn.com/v/12.1.2/72x72/" + getTwenmojiHashFromNativeEmoji(lastEmojiNative) + ".png"} alt={lastEmojiNative} className={cssStyle.emoji} />
    )
}
    ...
    return (
        <div className={cssStyle.containerInput + " " + (!enabled ? cssStyle.disabled : "")}>
            <div
                ...
                data-placeholder={enabled ? props.placeholder : tr("You can not write here.")}
