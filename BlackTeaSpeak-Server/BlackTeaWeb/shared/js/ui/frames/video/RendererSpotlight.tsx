...
class SpotlightGridController extends React.PureComponent<{ fontSize: number }, { layout: LayoutItem[] }> {
    private currentLayout: LayoutItem[] = [];
                                ...
                                >
                                    <GridLayout
                                        ...
                                        gridConfig={{
                                            ...
                                            margin: [this.props.fontSize * .5, this.props.fontSize * .5] as [number, number],
                                            maxRows: this.rowCount,
                                        }}

                                        onLayoutChange={newLayout => {
                                            this.currentLayout = newLayout.map(entry => ({ ...entry }));
                                            this.setState({ layout: this.currentLayout });
                                        ...
                                        resizeConfig={{ handles: ["ne", "nw", "se", "sw"] }}
                                    >
    ...
    addBox(key: string) {
        const newLayout = _.cloneDeep(this.currentLayout) as LayoutItem[];

        let newTile: LayoutItem = { i: key, w: kGridScaleFactor, h: kGridScaleFactor, x: 0, y: 0 };
        ...
        calculateNewTile:
            if(newLayout.length === 0) {
                ...
                {
                    let biggest: LayoutItem = newLayout[0];
                    for(const entry of newLayout) {
