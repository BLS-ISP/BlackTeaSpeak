...
function renderToolItems() : MenuBarEntry[] {
    ...
    if(__build.target === "web") {
        ...
        items.push({
            type: "normal",
            label: tr("Whisper Targets"),
            icon: ClientIcon.ToggleWhisper,
            click: () => global_client_actions.fire("action_open_window", { window: "server-whisper-targets" }),
            disabled: !currentConnectionConnected
        });
    }
...
function renderHelpItems() : MenuBarEntry[] {
    ...
    items.push({
        type: "normal",
        label: __build.target === "web" ? tr("About BlackTeaSpeak Web") : tr("About TeaClient"),
...
function updateMenuBar() {
    pendingMenuBarUpdate = true;
    if(menuBarUpdateScheduled) {
        return;
    }

    menuBarUpdateScheduled = true;
    requestAnimationFrame(() => {
        void flushPendingMenuBarUpdates();
    });
}

let pendingMenuBarUpdate = false;
let menuBarUpdateScheduled = false;
let menuBarUpdateRunning = false;

async function flushPendingMenuBarUpdates() {
    if(menuBarUpdateRunning) {
        return;
    }

    menuBarUpdateRunning = true;
    try {
        while(pendingMenuBarUpdate) {
            pendingMenuBarUpdate = false;
            await doUpdateMenuBar();
        }
    } finally {
        menuBarUpdateRunning = false;
        menuBarUpdateScheduled = false;

        if(pendingMenuBarUpdate) {
            updateMenuBar();
        }
    }
}
