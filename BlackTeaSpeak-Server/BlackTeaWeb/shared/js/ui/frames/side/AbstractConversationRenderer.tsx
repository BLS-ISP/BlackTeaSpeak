...
const TimestampEntry = (props: { timestamp: Date, refDiv: React.Ref<HTMLDivElement> }) => {
    ...
    const triggerRevision = useReducer((value: number) => value + 1, 0)[1];
    ...
    }, [ props.timestamp, triggerRevision, update ]);
...
const LoadOderMessages = (props: { events: Registry<AbstractConversationUiEvents>, chatId: string, state: ChatHistoryState | "error", errorMessage?: string, retryTimestamp?: number, timestamp: number | undefined }) => {
    const shouldThrottle = props.state === "available" && Date.now() < props.retryTimestamp;
    const triggerRevision = useReducer((value: number) => value + 1, 0)[1];
    ...
    useEffect(() => {
        ...
        const timeout = setTimeout(() => triggerRevision(), props.retryTimestamp - Date.now());
        return () => clearTimeout(timeout);
    }, [ props.retryTimestamp, shouldThrottle, triggerRevision ]);
                ...
                <div
                    ...
                    onScroll={() => {
                        ...
                        if(firstMessageTimestamp && top <= 20 && this.state.historyState === "available" && Math.max(this.scrollHistoryAutoLoadThrottle, this.state.historyRetryTimestamp) < Date.now()) {
                            ...
                            if(this.scrollOffset === "bottom" || (typeof this.scrollOffset === "number" && this.scrollOffset > top)) {
...
export const ConversationPanel = React.memo((props: {
    ...
    const lastChatBoxEnabled = useRef<boolean>(false);

    const refChatBox = useRef<ChatBox>();

    const updateChatBox = () => {
        const nextEnabled = currentChat.current.id !== "unselected" && chatEnabled.current;
        if(lastChatBoxEnabled.current === nextEnabled) {
            return;
        }

        lastChatBoxEnabled.current = nextEnabled;
        refChatBox.current?.setState({ enabled: nextEnabled });
    };
    ...
    useEffect(() => {
        const chatBox = refChatBox.current;
        if(!chatBox) {
            return;
        }

        return chatBox.events.on("notify_typing", () => props.events.fire("action_self_typing", { chatId: currentChat.current.id }));
    }, [ props.events ]);
