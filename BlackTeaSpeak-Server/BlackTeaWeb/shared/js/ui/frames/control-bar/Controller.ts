...
export function initializeControlBarController(events: Registry<ControlBarEvents>, mode: ControlBarMode) : InfoController {
    ...
    events.on("action_open_whisper_targets", () => {
        const connection = infoHandler.getCurrentHandler();
        if(!connection || !connection.connected) {
            createErrorModal(tr("You have to be connected"), tr("You have to be connected!")).open();
            return;
        }

        global_client_actions.fire("action_open_window", {
            window: "server-whisper-targets",
