...
const MicrophoneButton = () => {
    const events = useContext(Events);
    const showWhisperTargets = __build.target === "web";
    ...
    if(state === "muted") {
        return (
            <Button switched={true} colorTheme={"red"} autoSwitch={false} iconNormal={ClientIcon.InputMuted} tooltip={tr("Unmute microphone")}
                ...
                />
                {showWhisperTargets ? <DropdownEntry
                    icon={ClientIcon.ToggleWhisper}
                    text={<Translatable>Whisper targets</Translatable>}
                    onClick={() => events.fire("action_open_whisper_targets", {})}
                /> : undefined}
