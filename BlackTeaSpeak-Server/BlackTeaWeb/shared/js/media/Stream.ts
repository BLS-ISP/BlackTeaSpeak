...
export async function requestMediaStream(deviceId: string | undefined, groupId: string | undefined, type: MediaStreamType) : Promise<MediaStream | InputStartError> {
    ...
    if(type === "video") {
        const maxWidth = settings.getValue(Settings.KEY_VIDEO_DEFAULT_MAX_WIDTH);
        const maxHeight = settings.getValue(Settings.KEY_VIDEO_DEFAULT_MAX_HEIGHT);
        if (maxWidth > 0) {
            constrains.width = { ideal: maxWidth };
        }
        if (maxHeight > 0) {
            constrains.height = { ideal: maxHeight };
        }
        constrains.frameRate = { ideal: 60 };
    }
...
export async function queryMediaPermissions(type: MediaStreamType, changeListener?: (value: PermissionState) => void) : Promise<PermissionState> {
    if('permissions' in navigator && 'query' in navigator.permissions) {
        try {
            const result = await navigator.permissions.query({ name: (type === "audio" ? "microphone" : "camera") as PermissionName });
            if(changeListener) {
