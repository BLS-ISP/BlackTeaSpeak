...
function normalizeArrayBuffer(message: ArrayBufferLike | ArrayBufferView): ArrayBuffer {
    if(ArrayBuffer.isView(message)) {
        return message.buffer.slice(message.byteOffset, message.byteOffset + message.byteLength) as ArrayBuffer;
    }

    const source = message as ArrayBufferLike;
...
export function sha1(message: string | ArrayBufferLike | ArrayBufferView) : PromiseLike<ArrayBuffer> {
    if(!(typeof(message) === "string" || ArrayBuffer.isView(message) || typeof message?.byteLength === "number")) throw "Invalid type!";

    let buffer = typeof(message) === "string" ? encode_text(message) : normalizeArrayBuffer(message);
