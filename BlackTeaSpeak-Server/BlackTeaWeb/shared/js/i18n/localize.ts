...
export namespace config {
    ...
    export function repository_config() {
        ...
        if(config.repositories.length == 0) {
            const defaultRepositoryUrl = settings.getValue(Settings.KEY_I18N_DEFAULT_REPOSITORY);
            const repositoryUrl = __build.target === "web" && defaultRepositoryUrl === "https://web.blackteaspeak.de/i18n/"
                ? "i18n"
                : defaultRepositoryUrl;

            load_repository(repositoryUrl).then(repo => {
...
export async function initializeI18N() {
    ...
    // await load_file("http://localhost/home/BlackTeaSpeak/BlackTeaSpeak/Web-Client/web/environment/development/i18n/de_DE.translation");
    // await load_file("http://localhost/home/BlackTeaSpeak/BlackTeaSpeak/Web-Client/web/environment/development/i18n/test.json");
}
