...
export class WrappedWebSocket {
    ...
    private normalizeSocketMessage(message: ArrayBufferLike | ArrayBufferView): ArrayBuffer | Uint8Array<ArrayBuffer> {
        if(ArrayBuffer.isView(message)) {
            if(message.buffer instanceof ArrayBuffer) {
                return message as Uint8Array<ArrayBuffer>;
            }

            const buffer = new Uint8Array(message.byteLength);
            buffer.set(new Uint8Array(message.buffer, message.byteOffset, message.byteLength));
            return buffer;
        }
    ...
    sendMessage(message: string | ArrayBufferLike | Blob | ArrayBufferView) {
        if(typeof message === "string") {
            ...
            this.bytesSend += message.size;
        } else {
            this.bytesSend += message.byteLength;
        }

        this.socket.send(typeof message === "string" || message instanceof Blob ? message : this.normalizeSocketMessage(message));
    }
}