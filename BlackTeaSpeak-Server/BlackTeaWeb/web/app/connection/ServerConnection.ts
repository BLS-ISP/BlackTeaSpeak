...
export class ServerConnection extends AbstractServerConnection {
    ...
    async connect(address : ServerAddress, handshake: HandshakeHandler, timeout?: number) : Promise<void> {
        ...
        const isLoopbackAddress = address.host === "localhost" || address.host === "127.0.0.1" || address.host === "::1";
        ...
        this.socket.callbackDisconnect = (code, reason) => {
            ...
            this.client.handleDisconnect(reason === "connection-ping-timeout"
                ? DisconnectReason.CONNECTION_PING_TIMEOUT
    ...
    private handleServerInit() {
        ...
        this.client.serverFeatures.awaitFeatures().then(succeeded => {
            ...
            if(this.client.serverFeatures.supportsFeature(ServerFeature.VIDEO, 1)) {
                ...
                logInfo(LogCategory.NETWORKING, tr("Server does not advertise BlackTeaWeb RTC support. Continuing without voice/video negotiation."));
    ...
    getServerType(): "blackteaspeak" | "teamspeak" | "unknown" {
        /* It's simple. Only BlackTeaSpeak support web clients */
        return "blackteaspeak";
    }
}