...
export class RtpVoiceConnection extends AbstractVoiceConnection {
    ...
    private get serverConnectionHandle(): AbstractServerConnection {
        return this.rtcConnection.getConnection();
    }
    ...
    private emitWhisperTargetChanged(oldTarget: WhisperTarget | undefined, newTarget: WhisperTarget | undefined) {
        this.events.fire("notify_whisper_target_changed", {
            oldTarget: cloneWhisperTarget(oldTarget),
            newTarget: cloneWhisperTarget(newTarget)
        });
    }
        ...
        if(recorder) {
            ...
            await recorder.ownRecorder(new class extends ConnectionRecorderProfileOwner {
                getConnection(): ConnectionHandler {
                    return rtpConnection.serverConnectionHandle.client;
                }
    ...
    private handleRecorderStop(reason: string) {
        const chandler = this.serverConnectionHandle?.client;
    ...
    private handleRecorderStart() {
        const chandler = this.serverConnectionHandle?.client;
        if(!chandler) {
            logWarn(LogCategory.VOICE, tr("Received local voice started event without an active connection handler."));
            return;
        }
    ...
    async startWhisper(target: WhisperTarget): Promise<void> {
        ...
        const previousTarget = cloneWhisperTarget(this.whisperTarget);
        this.whisperTarget = Object.assign({ canceled: false }, target);
        this.emitWhisperTargetChanged(previousTarget, this.whisperTarget);
        try {
    ...
    stopWhisper() {
        if(this.whisperTarget) {
            ...
            this.whisperTargetInitialize = undefined;
            this.emitWhisperTargetChanged(previousTarget, undefined);
            this.serverConnectionHandle.send_command("whispersessionreset").catch(error => {
    ...
    private updateSpeakerState() {
        const newState = this.serverConnectionHandle.client.isSpeakerMuted() || this.serverConnectionHandle.client.isSpeakerDisabled();
