...
async function performHTTPSTransfer(transfer: FileTransfer, body: FormData | undefined) : Promise<Response> {
    try {
        const response = await fetch(generateTransferURL(transfer), {
            ...
            body: body,
            headers: {
                /* for legacy BlackTeaSpeak servers (prior to 1.4.15) */
        ...
        if(!response.ok) {
            throw (response.type == 'opaque' || response.type == 'opaqueredirect' ? "invalid cross origin flag! May target isn't a BlackTeaSpeak server?" : response.statusText || "response is not ok");
        }
...
async function downloadFileDownload(transfer: FileDownloadTransfer, target: DownloadTransferTargetImpl) {
    const url = generateTransferURL(transfer, target.fileName);
...
class TextTransferSourceImpl extends TextTransferSource {
    ...
    getArrayBuffer() : ArrayBuffer {
        ...
        const encoded = encoder.encode(this.text);
