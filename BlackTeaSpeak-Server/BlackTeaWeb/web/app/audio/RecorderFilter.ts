...
export class JThresholdFilter extends JAbstractFilter<GainNode> implements ThresholdFilter {
    ...
    private analyzeAnalyseBuffer() {
        ...
        this.audioAnalyserNode.getByteTimeDomainData(this.analyseBuffer as Uint8Array<ArrayBuffer>);

        const rawLevel = JThresholdFilter.calculateAudioLevel(this.analyseBuffer, this.audioAnalyserNode.fftSize, 0, 0);

        let smooth;
        if(rawLevel > this.currentLevel)
            smooth = this.smoothAttack;
        else
            smooth = this.smoothRelease;

        this.currentLevel = this.currentLevel * smooth + rawLevel * (1 - smooth);
