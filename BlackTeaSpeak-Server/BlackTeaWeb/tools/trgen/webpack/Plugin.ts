...
export default class TranslateableWebpackPlugin {
    ...
    apply(compiler: webpack.Compiler) {
        compiler.hooks.thisCompilation.tap("TranslateableWebpackPlugin", compilation => {
            compilation.hooks.processAssets.tap({
                name: "TranslateableWebpackPlugin",
                stage: webpack.Compilation.PROCESS_ASSETS_STAGE_ADDITIONAL,
            }, () => {
                const payload = JSON.stringify(this.translations);
                compilation.emitAsset(this.config.assetName, new webpack.sources.RawSource(payload));
            });
        });
