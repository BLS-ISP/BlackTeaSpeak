...
HtmlWebpackInlineSourcePlugin.prototype.processTag = function (compilation, regex, tag, filename) {
    ...
    if (assetUrl) {
        ...
        const asset = getAssetByName(compilation, assetName);
        if (asset) {
...
function getAssetByName (compilation, assetName) {
    const asset = compilation.getAssets().find(entry => path.posix.relative('', entry.name) === assetName);
    return asset ? asset.source : undefined;
}
